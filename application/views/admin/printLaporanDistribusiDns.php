    ...
    <body onload="window.print()">
        ...
        <table class="table table-bordered table-hover table-striped">
            <tr>
                ...
                <th>ID News</th>
                <th>ID Donasi</th>
                <th>Judul Rilis News</th>
                <th>Tgl Rilis News</th>
                <th>Tgl Distribusi</th>
                <th>Nominal Distribusi</th>
            </tr>
            <?php 
        $ttl = 0; $no=1; foreach ($dataFilter as $inv):
              $subtotal = $inv->nominal_salur  ;
            //   $total += $subtotal + $psn->ongkir;
              $ttl += $subtotal ;
                 ?>
            <tr>
                <td><?php echo $no++ ?></td>
                <td><?php echo $inv->kd_news ?></td>
                <td><?php echo $inv->id_donasi?></td>
                <td><?php echo $inv->judul_news ?></td>
                <td><?php echo format_indoTgl ($inv->tgl_post) ?></td>
                <td><?php echo format_indoTgl ($inv->tgl_distribusi) ?></td>
                <td>Rp <?php echo number_format ($inv->nominal_salur,0,',','.') ?></td>
            ...
            <?php endforeach; ?>
            <tr>
                <td colspan="6" align="right">Total Penyaluran</td>
